...
/**
 * deprecated Use {@link StreamViewer}
 * 
 ...
 * @version $Id: ImageStreamViewer.java,v 1.17 2007/06/12 16:51:56 crueda Exp $
 */
public class ImageStreamViewer extends JPanel {
	...
	private ViewControlsPanel viewControlsPanel;
...
//				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS
		...
		viewControlsPanel = new ViewControlsPanel();
		viewControlsPanel.setImageStreamViewer(this);
	...
	/**
	 * Sets the stream definition.
	 */
	public void setStreamDef(StreamDef streamDef) {
		assert streamDef != null;
		if (this.streamDef != null) {
			System.out.println("ImageStreamViewer.setStreamDef: Second call ignored: only one stream can be processed");
			return;
		}
		if ( !isOverview ) {
			channelNameLabel.setText(streamDef.getName());
		}
		this.streamDef =  streamDef;
		this.channelDef = this.streamDef.getChannelDef();
	}
	...
	public void setVectors(List<VectorInfo> vectors) {
		...
		if ( vectors != null && vectors.size() >= 1 ) {
			...
			System.out.println("ImageStreamViewer: WARNING: No vector data!");
		}
	...
	public void setShowVector(String vectorName, boolean show) {
		if ( show ) {
			...
			if ( vectorInfo != null ) {
				...
				System.out.println("ImageStreamViewer: setShowVector: unexistent object: " +vectorName);
			}
		}
		else {
	...
	void _save() {
		if ( gscon == null ) {
			System.out.println("ImageStreamViewer._save: no server connection");
			return;
		}
		
		// FIXME gscon.getSavedURL expects streamID
		String saved = gscon.getSavedURL(channelDef.getID());
		if (saved == null) {
			return;
		}
		try {
	...
	public void writeConnectionStatus(Color bgcolor, String text) {
	...
	public synchronized void setReferenceSpace(Rectangle referenceSpace) {
		System.out.println("ImageStreamViewer: Setting reference space = " + referenceSpace);
	...
	public synchronized void newFrame(FrameStatus frameStatus) {
		...
		Utils.log("ImageStreamViewer: newFrame: " +frameStatus);
	...
	public synchronized void addImage(IImage img) {
		...
		StreamStatus streamStatus = gscon.getStreamStatus(streamID);
		
		assert streamStatus != null || Utils.fail(
				"No StreamStatus for streamID=" +streamID+ "\n" +
				"gscon=" +gscon+ "\n" +
				"getStreamDefs=" +gscon.getStreamDefs()+ "\n" +
				"getStreamStatuses=" +gscon.getStreamStatuses())
		;

		if (!isOverview) {
			ChannelDef channelDef = gscon.getChannelDefForStream(streamID); 
		...
		FrameStatus frameStatus = gscon.getFrameStatus(streamID, frameID);

		if ( streamDef == null ) {
			setStreamDef(gscon.getStreamDef(streamID));
//			setChannelDef(gscon.getChannelDefForStream(streamID));
		}
...
//		assert frameStatus != null;
		...
		if (frameStatus.completed()) {
			...
			imgPanel.drawLine("line:" +frameID + "_last", img.getX(),
					frameStatus.getLastRow(), img.getX() + psx * line_width - 1,
					frameStatus.getLastRow(), Color.YELLOW);
		}
	...
	public synchronized void loadBackgroundImage(URL url) {
		try {
			...
			System.out.println("ImageStreamViewer: loadBackgroundImage: error: " + ex.getMessage());
		}
	}
	...
	private static String _getFrameTip(FrameStatus frameStatus) {
		...
		return "<html>" 
				+ "<b>Stream: ID = " +streamDef.getStreamID() + "</b><br>"
				+ "<b>Channel: ID = " +channelDef.getID() + "</b><br>"
